...
// Define the queue and task parameters
typedef struct {
  uint8_t* mcubuf = NULL;
  uint_fast16_t x = 0;
  uint_fast16_t y = 0;
...
// Output task to handle MJPEG decompression and rendering
...
// Start multitasking for MJPEG decoder
// void TJpgD::multitask_begin() {
//   param.sem = xQueueCreate(queue_max + 1, sizeof(queue_t*));

//   xTaskCreatePinnedToCore(task_output, "task_output", 4096, &param, 2, &param.task, 0);
// }

// End multitasking for MJPEG decoder
void TJpgD::multitask_end() {
  queue_t* q = NULL;
  if (param.sem) {
    xQueueSend(param.sem, &q, portMAX_DELAY);
  }
  vTaskDelay(20 / portTICK_PERIOD_MS);  // Wait for task to finish
}

// Updated reset_decoder function to ensure proper synchronization
void TJpgD::reset_decoder() {
  Serial.println(F("Resetting decoder and clearing queue..."));

  // Properly delete the task if it exists
  if (param.task != NULL) {
    vTaskDelete(param.task);
    param.task = NULL;
  }

  // Delete and recreate the queue
  if (param.sem != NULL) {
    vQueueDelete(param.sem);
  }
  param.sem = xQueueCreate(queue_max + 1, sizeof(queue_t*));

  // Reset internal states related to the queue and decoder
  qidx = 0;
  mcuidx = 0;
  dcv[2] = dcv[1] = dcv[0] = 0;  // Reset DC values

  // Restart the task after a delay to ensure proper cleanup
  vTaskDelay(100 / portTICK_PERIOD_MS);  // Increased delay to ensure task is fully cleaned up
  xTaskCreatePinnedToCore(task_output, "task_output", 4096, &param, 2, &param.task, 0);

  Serial.println(F("Decoder and task reset complete."));
}

// Start multitasking for MJPEG decoder with improved error handling
void TJpgD::multitask_begin() {
  // Create queue for task synchronization
  if (param.sem == NULL) {
    param.sem = xQueueCreate(queue_max + 1, sizeof(queue_t*));
  }
...
// Improved MJPEG decompression with multitasking
TJpgD::JRESULT TJpgD::decomp_multitask(
  ...
  // Dynamically allocate workbuf on the heap
  uint8_t* workbuf = (uint8_t*)malloc(768);
  if (workbuf == nullptr) {
    Serial.println(F("Error: Failed to allocate workbuf!"));
  ...
  int retry_count = 0;  // Track retries

  // Main loop: process the image one MCU (Minimum Coded Unit) at a time
  do {  // Vertical loop of MCUs
    ...
    do {  // Horizontal loop of MCUs
      ...
      if (!q) {
        Serial.println(F("Error: q is null!"));
        free(workbuf);
        return TJpgD::JDR_INTR;
      }

      if (!q->queue) {
        // Send MCU data to the queue
        q->mcubuf = mcubufs[mcuidx];
        q->x = x;
        q->y = y;
        q->queue = true;

        // Ensure queue send succeeds
        if (xQueueSend(param.sem, &q, portMAX_DELAY) != pdTRUE) {
          Serial.println(F("Error: xQueueSend failed!"));
          free(workbuf);
          return TJpgD::JDR_INTR;
        }
        ...
        // Timeout handling for queue processing
        unsigned long start_time = millis();
        while (ql && ql->queue) {
          if (millis() - start_time > 300) {  // Further increase timeout to 300 ms
            Serial.println(F("Timeout waiting for ql->queue. Resetting queue..."));
            xQueueReset(param.sem);  // Resetting the queue to avoid deadlock.
            reset_decoder();         // Reset the decoder state to ensure no lingering issues.
            free(workbuf);           // Free workbuf to prevent memory leak
            return TJpgD::JDR_INTR;  // Skip this frame
          }
          taskYIELD();  // Allow the RTOS to switch tasks
        }
    ...
    // Handle line function and final processing
    if (linefunc && (yidx == lineskip || y == lasty)) {
      unsigned long start_time = millis();
      while (ql && ql->queue) {
        if (millis() - start_time > 300) {  // Further increase timeout to 300 ms
          Serial.println("Timeout waiting for ql->queue in linefunc");
          free(workbuf);
          ql->queue = false;
          return TJpgD::JDR_INTR;  // Skip this frame
        }
      ...
      while (xQueueReceive(param.sem, &qtmp, 0)) {
        if (!qtmp) {
          Serial.println(F("Error: qtmp is null!"));
          break;
        }
      ...
      if (ql != nullptr) {
        ...
        if (xQueueSend(param.sem, &ql, portMAX_DELAY) != pdTRUE) {
          Serial.println(F("Error: xQueueSend for ql failed!"));
          free(workbuf);
          return TJpgD::JDR_INTR;
        }
      }
